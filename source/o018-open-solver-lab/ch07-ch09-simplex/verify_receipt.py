"""Bangun atau periksa receipt laboratorium simpleks Bab 7--9.
...
def _validate_log(results_payload: bytes) -> None:
    ...
    check_line = (
        ...
        "plots=3 exercises=38 solver_calls=16"
    )
    required_counts = {
        check_line: 2,
        "unittest: 28 passed, 0 failed, 0 errors": 2,
        "visual_qa: 3 SVG diperiksa pada raster 1440x1020": 1,
        "source_math_corrections: 0": 1,
        "source_manual_title_divergences: 3": 1,
        "implementation_corrections: 1": 1,
    }
    for text, count in required_counts.items():
        if log.count(text) != count:
            raise RuntimeError(f"bukti log tidak lengkap untuk {text!r}: {log.count(text)} != {count}")
...
def build_receipt() -> dict[str, Any]:
    ...
    artifacts = [_record(path, path.relative_to(HERE).as_posix()) for path in artifact_paths]
    ...
    return {
        ...
        "verification": {
            ...
            "chapter_counts": first_results["summary"]["chapter_counts"],
            "correction_count": first_results["summary"]["correction_count"],
            "exercise_count": first_results["summary"]["exercise_count"],
            "implementation_corrections": ["escaped_css_braces_before_first_successful_svg_render"],
            "maximum_solver_violation": first_results["summary"]["maximum_solver_violation"],
            ...
            "solver_call_count_per_run": first_results["summary"]["solver_call_count"],
            "solver_termination_counts": first_results["summary"]["solver_termination_counts"],
            "source_manual_title_divergence_count": first_results["summary"]["source_manual_title_divergence_count"],
            "tests_failed_per_run": 0,
            "tests_passed_per_run": 28,
            "underdetermined_count": first_results["summary"]["underdetermined_count"],
            "unittest_runs": 2,
            "verification_date": "2026-08-22",
            "verified_count": first_results["summary"]["verified_count"],
            "visual_qa_raster": "1440x1020",
            "visual_qa_svg_count": 3,
        },
    }
...
def main() -> int:
    ...
    if args.write:
        ...
        print(
            "RECEIPT_WRITE_OK "
            f"artifacts={len(json.loads(payload)['artifacts'])} bytes={len(payload)} "
