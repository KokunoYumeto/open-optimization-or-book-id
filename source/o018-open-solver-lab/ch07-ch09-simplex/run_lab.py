"""Jalankan laboratorium simpleks Bab 7--9 secara deterministik.
...
def compare_outputs(
    ...
    if not output_path.is_file():
        ...
        failures.append(
            "results_mismatch="
            f"{hashlib.sha256(output_path.read_bytes()).hexdigest()}!="
            f"{hashlib.sha256(results_payload).hexdigest()}"
        )
    actual_names = {path.name for path in plots_dir.glob("*.svg")} if plots_dir.is_dir() else set()
    if actual_names != set(plots):
        failures.append(f"plot_inventory_mismatch=expected:{sorted(plots)};actual:{sorted(actual_names)}")
    for filename, expected in plots.items():
...
def main() -> int:
    ...
    unexpected = {path.name for path in args.plots_dir.glob("*.svg")} - set(plots)
    if unexpected:
        raise RuntimeError("direktori plot memuat SVG tak terkelola: " + ", ".join(sorted(unexpected)))
