"""Jalankan laboratorium analisis sensitivitas secara deterministik.
...
def compare_outputs(
    ...
    if actual_names != set(plots):
        failures.append(
            ...
            f"expected:{sorted(plots)};actual:{sorted(actual_names)}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output", type=Path, default=HERE / "results.json"
    )
    parser.add_argument(
        "--plots-dir", type=Path, default=HERE / "plots"
    )
    parser.add_argument(
        "--check", action="store_true", help="bandingkan byte tanpa menulis"
    )
...
def main() -> int:
    ...
    if args.check:
        failures = compare_outputs(
            args.output, args.plots_dir, payload, plots
        )
        if failures:
    ...
    if unexpected:
        raise RuntimeError(
            "direktori plot memuat SVG tak terkelola: "
