"""Bangun atau periksa receipt laboratorium analisis sensitivitas.
...
def _validate_verification_log(results_payload: bytes) -> None:
    ...
    check_line = (
        ...
        f"results_sha256={digest} plots=3 exercises=12 solver_calls=29"
    )
    required_counts = {
        check_line: 2,
        "unittest: 23 passed, 0 failed, 0 errors": 2,
        "visual_qa: 3 SVG diperiksa pada raster 1440x1020": 1,
        "source_math_corrections_preserved: 3": 1,
        "o018_math_corrections: 0": 1,
        "underdetermined_exercises: 1 (10.7; fail closed)": 1,
    }
    for text, count in required_counts.items():
        observed = log.count(text)
        if observed != count:
            raise RuntimeError(
                f"bukti log tidak lengkap untuk {text!r}: "
                f"{observed} != {count}"
            )
...
def build_receipt() -> dict[str, Any]:
    ...
    if failures:
        raise RuntimeError(
            "keluaran disk tidak cocok: " + " | ".join(failures)
        )
    ...
    return {
        ...
        "verification": {
            ...
            "correction_ids": [
                item["id"] for item in data["corrections"]
            ],
            ...
            "o018_math_correction_count": first_results["summary"][
                "o018_math_correction_count"
            ],
            ...
            "solver_termination_counts": first_results["summary"][
                "solver_termination_counts"
            ],
            "source_math_corrections_preserved": len(
                data["corrections"]
            ),
            "tests_failed_per_run": 0,
            "tests_passed_per_run": 23,
            "underdetermined_exercises": [
                item["exercise_id"]
                for item in data["underdetermined_exercises"]
            ],
            "unittest_runs": 2,
