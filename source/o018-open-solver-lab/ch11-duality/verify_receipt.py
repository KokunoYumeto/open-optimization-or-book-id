"""Bangun atau periksa receipt laboratorium dualitas.
...
def _validate_verification_log(results_payload: bytes) -> None:
    ...
    check_line = (
        ...
        f"results_sha256={digest} plots=1 exercises=17 solver_calls=36"
    )
    required_counts = {
        check_line: 2,
        "unittest: 29 passed, 0 failed, 0 errors": 2,
        "visual_qa: 1 SVG diperiksa pada raster 1440x1020": 1,
        "source_math_corrections_preserved: 3": 1,
        "o018_math_corrections: 0": 1,
        "high_confidence_upstream_defects: 2": 1,
        "underdetermined_exercises: 0": 1,
        "solver_classifications: optimal=32 infeasible=3 unbounded=1": 1,
    ...
    for text, count in required_counts.items():
        ...
        if observed != count:
            raise RuntimeError(
                f"bukti log tidak lengkap untuk {text!r}: {observed} != {count}"
            )
...
def build_receipt() -> dict[str, Any]:
    ...
    artifacts = [
        _record(path, path.relative_to(HERE).as_posix())
    ...
    return {
        ...
        "verification": {
            ...
            "correction_ids": [item["id"] for item in data["corrections"]],
            "exercise_count": first_results["summary"]["exercise_count"],
            "high_confidence_upstream_defect_ids": [
                item["id"] for item in data["upstream_defects"]
            ],
            ...
            "proof_only_exercises": data["proof_only_exercises"],
            ...
            "source_math_corrections_preserved": len(data["corrections"]),
            "tests_failed_per_run": 0,
            "tests_passed_per_run": 29,
            "underdetermined_exercises": [],
            "unittest_runs": 2,
...
def parse_args() -> argparse.Namespace:
    ...
    mode.add_argument("--check", action="store_true", help="periksa receipt")
