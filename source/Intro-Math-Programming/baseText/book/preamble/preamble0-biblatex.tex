%%% BibLaTeX setup --> see also
%%% /open-optimization-bibliography/test/bibliography-biblatex.tex.

\usepackage{csquotes}   % to work around a bug in pre-2018 biblatex, https://github.com/plk/biblatex/issues/737
\usepackage[backend=biber,giveninits=true,maxnames=100,maxcitenames=5,sorting=nyt,backref=true]{biblatex}
\addbibresource{../../../open-optimization-bibliography/bib/references.bib}
% The pinned Book 1 source has a small book-local bibliography that is not
% duplicated in the shared bibliography repository.
\addbibresource{references.bib}
% book1-main.tex is built from baseText/book, so these metadata paths must
% traverse to the sibling optimization directory.
\addbibresource{../optimization/figures/figures-static/00_METADATA.bib}
\addbibresource{../optimization/figures/figures-source/00_METADATA.bib}
% Four citations used by Chapter 1 are absent from the pinned upstream
% bibliography closure.  Keep their source-backed repair separate and explicit.
\addbibresource{preamble/id-ID-source-corrections.bib}

% print url only if no doi.  https://tex.stackexchange.com/questions/424774/print-url-only-if-doi-not-present
%\renewbibmacro*{doi+eprint+url}{%
%\printfield{doi}%
%\newunit\newblock%
%\iftoggle{bbx:eprint}{%
%\usebibmacro{eprint}%
%%     }{}%
%\newunit\newblock%
%\iffieldundef{doi}{%
%\usebibmacro{url+urldate}}%
%%         {}%
%%     }
% print url&eprint only if no doi. Adapted from above.
\renewbibmacro*{doi+eprint+url}{%
    \printfield{doi}%
    \newunit\newblock%
    \iffieldundef{doi}{%
      \iftoggle{bbx:eprint}{%
        \usebibmacro{eprint}%
      }{}%
    }{}%
    \newunit\newblock%
    \iffieldundef{doi}{%
        \usebibmacro{url+urldate}}%
        {}%
    }

\DeclareRobustCommand\footnotetextmetadata[1]{%
  \footnotetext{\citetitle*{#1}, from \citeurl{#1}.
    \citeauthor{#1}, \citedate{#1}.
  }
}

\usepackage{xargs}

% Adds graphic in figure environment and references it immediately.
\newcommandx{\refincludefigurestatic}[4][1=\relax,2={width=.4\linewidth},3=t, usedefault]{%
  \begin{figure}[#3]%
    \centering%
    \includegraphics[#2]{optimization/figures/figures-static/#4}\\
\centerline{\scriptsize
\copyright~\citeauthor{#4}\protect\footnotemark}
    \ifx#1\relax % Caption from TITLE field in .bib
      \caption{\citetitle*{#4}}%
    \else        % Caption provided as an argument to the macro
      \caption{#1}%
    \fi
    \label{fig:#4}%
  \end{figure}%
  \autoref{fig:#4}\footnotetextmetadata{#4}%
}

% Broken first definition of \includefigurestatic removed (had commented-out braces)
% The correct definition is below (after \includegraphicsource)

% Includes the graphic but not in a figure environment.  Adds proper credit citation below with footnote.
\newcommandx{\includegraphicstatic}[2][1={width=.4\linewidth}, usedefault]{%
    \begin{center}
    \includegraphics[#1]{optimization/figures/figures-static/#2}\\
    \end{center}
\centerline{\scriptsize 
\copyright~\citeauthor{#2}\protect\footnotemark}
  \footnotetextmetadata{#2}%
}

\newcommandx{\includegraphicsource}[2][1={width=.4\linewidth}, usedefault]{%
    \begin{center}
    \includegraphics[#1]{optimization/figures/figures-source/#2}\\
    \end{center}
\centerline{\scriptsize 
\copyright~\citeauthor{#2}\protect\footnotemark}
  \footnotetextmetadata{#2}%
}

\newcommandx{\includefigurestatic}[4][1=\relax,2={width=.4\linewidth},3=t, usedefault]{%
  \begin{figure}[#3]%
    \centering%
    \includegraphics[#2]{optimization/figures/figures-static/#4}\\
\centerline{\scriptsize
\copyright~\citeauthor{#4}\protect\footnotemark}
    \ifx#1\relax % Caption from TITLE field in .bib
      \caption{\citetitle*{#4}}%
    \else        % Caption provided as an argument to the macro
      \caption{#1}%
    \fi
    \label{fig:#4}%
  \end{figure}%
  \footnotetextmetadata{#4}%
}

\newcommandx{\includefiguresource}[4][1=\relax,2={width=.4\linewidth},3=t, usedefault]{%
  \begin{figure}[#3]%
    \centering%
    \includegraphics[#2]{optimization/figures/figures-source/#4}\\
\centerline{\scriptsize
\copyright~\citeauthor{#4}\protect\footnotemark}
    \ifx#1\relax % Caption from TITLE field in .bib
      \caption{\citetitle*{#4}}%
    \else        % Caption provided as an argument to the macro
      \caption{#1}%
    \fi
    \label{fig:#4}%
  \end{figure}%
  \footnotetextmetadata{#4}%
}

%%%
% Book-native figure macros.
% Use these for figures created for this book (by the author or contributors
% under contract).  They render the image WITHOUT a per-figure copyright line
% or attribution footnote: credit is recorded in 00_METADATA.bib and the
% figures are covered by the book's own license.
%%%

% Bare centered graphic from figures-static, no credit line.
\newcommandx{\includegraphicbook}[2][1={width=.4\linewidth}, usedefault]{%
    \begin{center}
    \includegraphics[#1]{optimization/figures/figures-static/#2}%
    \end{center}%
}

% Bare centered graphic from figures-source, no credit line.
\newcommandx{\includegraphicbooksource}[2][1={width=.4\linewidth}, usedefault]{%
    \begin{center}
    \includegraphics[#1]{optimization/figures/figures-source/#2}%
    \end{center}%
}

% Figure environment from figures-static, no credit line.  Caption defaults
% to the TITLE field in 00_METADATA.bib; label is fig:<key>.
\newcommandx{\includefigurebook}[4][1=\relax,2={width=.4\linewidth},3=t, usedefault]{%
  \begin{figure}[#3]%
    \centering%
    \includegraphics[#2]{optimization/figures/figures-static/#4}%
    \ifx#1\relax
      \caption{\citetitle*{#4}}%
    \else
      \caption{#1}%
    \fi
    \label{fig:#4}%
  \end{figure}%
}

% Figure environment from figures-source, no credit line.
\newcommandx{\includefigurebooksource}[4][1=\relax,2={width=.4\linewidth},3=t, usedefault]{%
  \begin{figure}[#3]%
    \centering%
    \includegraphics[#2]{optimization/figures/figures-source/#4}%
    \ifx#1\relax
      \caption{\citetitle*{#4}}%
    \else
      \caption{#1}%
    \fi
    \label{fig:#4}%
  \end{figure}%
}

%%
% Might include a version of this that will be prepped to create a bibliography of figure references at the end of the chapter or book.
%%

% Stray line removed - was causing compilation error
% \copyright~\hyperref[m0075_fEMS_Credit]{V.\ Blacus}  \href{https://creativecommons.org/licenses/by-sa/3.0/}{CC BY-SA 3.0} }

%%% Local Variables:
%%% mode: latex
%%% TeX-master: "open-optimization/open-optimization"
%%% End:
